...
| STALE | 一度も add されていない未追跡ファイルで `--days` より古いもの |

隠しディレクトリ（`~/.nvm` など）配下は既定で除外する。他人の clone であって
自分の作業ではないため。

コミットと stash の日数は git 自身の記録から取るので正確。

未コミット変更と未追跡ファイルの日数はファイルの mtime 由来で、**常に `~` を付ける**。
mtime が記録しているのは「最後に書き込まれた時刻」であって「作業した時刻」ではない。
`git checkout` / `git pull` / `git stash pop` はどれも、作業内容を変えずにファイルを
書き直す。**2年前の作業が1週間前に見えて `--days` で抑制されうる。**
これらの数字は測定値ではなく、年齢の下限として扱うこと。

mtime を読むファイルが存在しない場合（削除・リネーム）は最終コミット日で近似する。
黙って落とすことはしない。
